	        	   ...
	        	   for(var i in scope.reportData){
	        		   if(scope.reportData[i].status == 'New')
	          		    ...
	          		    if(scope.reportData[i].status == 'Pending')
	        		   	...
	        		   	scope.clientStatusData.push({key:scope.reportData[i].status,y:scope.reportData[i].ccounts});	
	        		   	scope.totalClients = parseInt(scope.totalClients)+parseInt(scope.reportData[i].ccounts);
	      	  		}
	        		scope.clientStatusData.push({key:'totalClients',y:scope.totalClients});
...
/* Paymode Collection Chart */
		 scope.id = this.officeIdCollection || 1;
		 /*var startdate1 = Date.today().clearTime().moveToFirstDayOfMonth();
		 scope.startdate = startdate1.toString("yyyy-mm-dd");*/
		 /*scope.startdate = "2017-09-01";
		 scope.enddate = "2017-09-19";*/
		 /*var startdate = new Date();
		 scope.current_date = startdate.getDate();
		 scope.enddate = "2017-09-19";*/
 ...
 //https://localhost:8877/obsplatform/api/v1/runreports/Paymode%20Collection%20Chart?R_endDate=2017-09-01&R_officeId=1&R_startDate=2017-09-19&genericResultSet=false
 //https://localhost:8877/obsplatform/api/v1/runreports/Paymode%20Collection%20Chart?R_endDate=2017-09-19&R_officeId=1&R_startDate=2017-09-01
             ...
             resourceFactory.runReportsResource.get({reportSource: 'Stock_Item_Details',R_officeId:1, genericResultSet:false} , function(data) 
            	...
            	if(count==data.length)
                 ...
                 for(var i in data){
                	 ...
                	 scope.disbursedData.push({key:data[i].disbursedAmount,y:data[i].amountToBeDisburse});
                 }
             });
   };
...
/* Stock Item Details */
            ...
            var colorArrayPie1 =['#f45656','#ffd145','#39b54a','#959595'];
