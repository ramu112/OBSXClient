(function(module) {
	mifosX.controllers = _
			.extend(
					module,
					{
						ViewClientController : function(scope, webStorage,
								routeParams, route, location, resourceFactory,
								paginatorService, http, $modal, dateFilter,
								API_VERSION, $rootScope, PermissionService,
								localStorageService, TENANT) {
							...
							scope.ipstatus;
							scope.ipId;
							scope.walletconfig = webStorage
									.get('is-wallet-enable');
							scope.propertyMaster = webStorage
									.get("is-propertycode-enabled");
							...
							resourceFactory.oneTimeSaleResource.getOneTimeSale({clientId : routeParams.id},function(data) {
								//scope.onetimesales = angular.copy(data.oneTimeSaleData);
							...
							resourceFactory.getOrderResource.getAllOrders({clientId : routeParams.id},function(ordersData) {
								//scope.orders = angular.copy(ordersData.clientOrders);
							...
							function  suspendCSPopupController($scope, $modalInstance,clientServiceId) {
								$scope.suspendCS = function () {
									scope.suspendFormData = {};
									scope.suspendFormData.locale = $rootScope.locale.code;
							...
							function  reactiveCSPopupController($scope, $modalInstance,clientServiceId) {
							...
							function  terminateCSPopupController($scope, $modalInstance,clientServiceId) {
							...
							scope.suspendOrder = function (orderId){
				             ...
				             var suspendOrderController = function ($scope, $modalInstance,orderId) {
				            	 ...
				            	 $scope.maxDate=new Date();
				            	  resourceFactory.OrderSuspensionResource.get(function(data) {
				                    ...
				                    scope.terminateOrderpopup = function(orderId){
										$modal.open({
											templateUrl : 'terminateOrderPopup.html',
											controller : terminateOrderPopupController,
											resolve : {
												orderId : function() {
													return orderId;
												}
									...
									var terminateOrderPopupController = function ($scope, $modalInstance,orderId) {
							    		
							            $scope.approveTerminate = function () {

							            	$scope.flagapproveTerminate=true;
							            	if(this.terminateOrderFormData == undefined || this.terminateOrderFormData == null){
							            		this.terminateOrderFormData = {};
							            	}
							            	resourceFactory.OrderTerminateResource.update({orderId: orderId} ,this.terminateOrderFormData, function(data) {              	
							            		resourceFactory.getSingleOrderResource.get({orderId: orderId} , function(data) {
							                        scope.orderPriceDatas= data.orderPriceData;
							                        scope.orderHistorydata=data.orderHistory;
							                        scope.orderData=data.orderData;
							...
							scope.routeToticket = function(clientId, ticketid) {
								location.path('/viewTicket/' + clientId + '/'
										+ ticketid);
							};

							scope.routeToFollowupTicket = function(clientId,
									ticketid) {
								location.path('/editTicket/' + clientId + '/'
										+ ticketid);
							};

							scope.routeToCloseTicket = function(clientId,
									ticketid) {
								location.path('/closeTicket/' + clientId + '/'
										+ ticketid);
							};
							...
							scope.routeTofinancial = function(transactionId,
									transtype, clientid) {
								if (transtype == 'INVOICE') {
									location.path('/viewfinancialtran/'
											+ transactionId + '/' + clientid);
								}
							};

							scope.routeToItemSale = function(onetimesaleid,
									clientid) {
								location.path('/viewonetimesale/'
										+ onetimesaleid + '/' + clientid);
							};

							scope.routeToCardDetails = function(clientid, id,
									cardType) {
								location.path('/viewcarddetails/' + clientid
										+ '/' + id + '/' + cardType);
							};

							scope.routeTotable = function(tableName, clientId,
									cardType) {
								location.path('/viewdatatableentry/'
										+ tableName + '/' + clientId + '/'
										+ cardType);
							};

							scope.routeToClientService = function(clientId, id) {
								location.path('/viewclientservice/' + clientId
										+ '/' + id);
							};

							scope.clientServiceActivation = function(
									clientServiceId) {
								scope.data = {};
								scope.data.clientId = parseInt(scope.clientId);
								resourceFactory.clientServiceActivationResource
										.save({
							...
							var bookOrder = PermissionService
									.showMenu('CREATE_ORDER')
									&& PermissionService.showMenu('READ_ORDER');
							var redemption = PermissionService
									.showMenu('CREATE_REDEMPTION');
							var riseTicket = PermissionService
									.showMenu('CREATE_TICKET')
									&& PermissionService
											.showMenu('READ_TICKET');
							var makePayment = PermissionService
									.showMenu('CREATE_PAYMENT')
									&& PermissionService
											.showMenu('READ_GETPAYMENT');
							var payInvoice = PermissionService
									.showMenu('CREATE_PAYMENT')
									&& PermissionService
											.showMenu('READ_GETPAYMENT')
									&& PermissionService
											.showMenu('READ_INVOICEMAP');
							var distribute = PermissionService
									.showMenu('CREATE_CREDITDISTRIBUTION')
									&& PermissionService
											.showMenu('READ_CREDITDISTRIBUTION');
							var postAdjustment = PermissionService
									.showMenu('CREATE_ADJUSTMENT')
									&& PermissionService
											.showMenu('READ_ADJUSTMENT');
							var doInvoice = PermissionService
									.showMenu('CREATE_INVOICE');
							var statement = PermissionService
									.showMenu('READ_BILLMASTER');
							var edit = PermissionService
									.showMenu('UPDATE_CLIENT');
							var acceptTransfer = PermissionService
									.showMenu('ACCEPTTRANSFER_CLIENT');
							var rejectTransfer = PermissionService
									.showMenu('REJECTTRANSFER_CLIENT');
							var undoTransfer = PermissionService
									.showMenu('WITHDRAWTRANSFER_CLIENT');

							var getDetails = function() {

								resourceFactory.clientResource
										.get(
												{
												...
												function(data) {
													...
													// adding account no and
													// name to display recent
													// clients in dashboard
													...
													scope.buttons = [
															{
																name : "button.addservice",
																href : "#/createclientservice",
																icon : "icon-plus-sign",
																ngShow : "cs"
															},
															/*{
													...
													resourceFactory.EventActionResource
															.get(
																	{
																		clientId : routeParams.id
																	},
													...
													resourceFactory.DataTablesResource
															.query(
																	{
																		apptable : 'm_client'
																	},
							...
							function  detailsCSPopupController($scope, $modalInstance,clientId,clientServiceId) {
								scope.ServiceParameterData = [];
							...
							scope.getAllProvisioningDetails = function() {
								...
								resourceFactory.provisioningDetailsMappingResource
										.query({
							...
							scope.reProcess = function(processId) {
								...
								resourceFactory.updateProvisioningMappingResource
										.update({
											'provisioningId' : processId
										}, {}, function(data) {
											/*
							...
							var ApproveConfirm = function($scope,
									$modalInstance) {
								...
								$scope.approveTerminate = function() {
									...
									resourceFactory.confirmProvisioningDetailsResource
											.update(
													{
														'provisioningId' : scope.provId
													},
													{},
													function(data) {
														/*
														 ...
														 */
														scope
																.getAllProvisioningDetails(scope.orderNumber);
														$modalInstance
																.close('delete');
													},
													function(errData) {
							...
							var SentMessageController = function($scope,
									$modalInstance) {
								...
								for ( var i in scope.sentMessagesData) {
									...
									if (scope.sentMessagesData[i].id == scope.provisioningDataId) {
										...
										try {
											var obj = JSON
													.parse(scope.sentMessagesData[i].sentMessage);
										...
										for ( var key in $scope.sentMessage) {
											if (key == "IP_ADDRESS") {
												var outerStr = $scope.sentMessage[key]
														.toString();
												$scope.sentMessage[key] = [];
												$scope.sentMessage[key].push({
													"key" : outerStr,
													"value" : ""
												});
												$scope.messageData
							...
							var AllocatePropertyController = function($scope,
									$modalInstance) {

								$scope.propertycodes = [];
								resourceFactory.propertydeviceMappingTemaplateResource
										.get({
											'clientId' : routeParams.id
										}, function(data) {
								...
								$scope.approveAllocate = function() {
									...
									resourceFactory.propertydeviceMappingResource
											.update(
													{
														'clientId' : routeParams.id
													},
													...
													function(data) {
														 ...
														 */
														resourceFactory.oneTimeSaleResource
																.getOneTimeSale(
																		{
																			clientId : routeParams.id
																		},
							...
							var Approve = function($scope, $modalInstance) {
								...
								$scope.accept = function(date) {
									...
									var reqDate = dateFilter(date,
											'dd MMMM yyyy');
									scope.formData.dateFormat = 'dd MMMM yyyy';
									scope.formData.systemDate = reqDate;
									resourceFactory.clientInvoiceResource
											.save(
													{
														'clientId' : routeParams.id
													},
							...
							var CancelPayment = function($scope,
									$modalInstance, getPaymentId) {
								...
								$scope.accept = function(cancelRemark) {
									...
									resourceFactory.cancelPaymentResource
											.update(
													{
														'paymentId' : paymentId
													},
							...
							scope.deleteChildsFromparent = function(childId) {
								...
								resourceFactory.clientParentResource.remove({
									clientId : routeParams.id,
									anotherresource : childId
								}, {}, function(data) {
							...
							var StatementPopController = function($scope,
									$modalInstance) {
								...
								$scope.acceptStatement = function() {
									...
									this.formData.locale = $rootScope.locale.code;
									var reqDate = dateFilter($scope.start.date,
											'dd MMMM yyyy');
									this.formData.dateFormat = 'dd MMMM yyyy';
									this.formData.dueDate = reqDate;
									resourceFactory.statementResource
											.save(
													{
														'clientId' : routeParams.id
													},
							...
							var redemptionPopController = function($scope,
									$modalInstance) {
								...
								$scope.acceptRedemption = function() {
									...
									resourceFactory.redemptionResource
											.save(
													this.formData,
													function(data) {
							...
							var ClientDeleteCtrl = function($scope,
									$modalInstance) {

								$scope.deleteClientConfirm = function() {
									resourceFactory.clientResource.remove({
										clientId : routeParams.id
									}, {}, function(data) {
							...
							var ClientUnassignCtrl = function($scope,
									$modalInstance) {
							...
							scope.getClientNotes = function() {
								if (PermissionService
										.showMenu('READ_CLIENTNOTE')) {
									resourceFactory.clientNotesResource
											.getAllNotes({
												clientId : routeParams.id
											}, function(data) {
							...
							scope.getClientIdentityDocuments = function() {
								...
								scope.offbtn = function() {
									...
									if (scope.taxExemption != 'N') {
										...
										resourceFactory.taxExemptionResource
												.update({
													clientId : routeParams.id
												}, obj, function(data) {
												});
										...
										.getAllClientDocuments(
												...
												function(data) {
													...
													for ( var i = 0; i < scope.identitydocuments.length; i++) {
														resourceFactory.clientIdentifierResource
																.get(
																		{
								...
								// parentClient
								resourceFactory.clientParentResource.get({
									clientId : routeParams.id
								}, function(data) {
									scope.parent = [];
							...
							scope.documnetsUploadsTabFun = function() {
								...
								if (PermissionService.showMenu('READ_DOCUMENT')) {
									resourceFactory.clientDocumentsResource
											.getAllClientDocuments({
												clientId : routeParams.id
											}, function(data) {
							...
							scope.creditCardDetailsTabFun = function() {
								...
								// credit card details
								resourceFactory.creditCardSaveResource
										.get(
												{
													clientId : routeParams.id
												},
												function(data1) {
													...
													for ( var i in scope.clientcarddetails) {
														...
														if (scope.clientcarddetails[i].type == 'CreditCard') {
															...
															var decrypted1 = CryptoJS.AES
																	.decrypt(
																			scope.clientcarddetails[i].cardNumber,
																			key);
															var cardNum = decrypted1
															...
															var decrypted2 = CryptoJS.AES
																	.decrypt(
																			scope.clientcarddetails[i].cardExpiryDate,
																			key);
															scope.clientcarddetails[i].cardExpiryDate = decrypted2
																	.toString(CryptoJS.enc.Utf8);
															...
															var decrypted1 = CryptoJS.AES
																	.decrypt(
																			scope.clientcarddetails[i].routingNumber,
																			key);
															var routingNumber = decrypted1
																	.toString(CryptoJS.enc.Utf8);
															var stars = "";
															for ( var j in routingNumber) {
																if (j >= 0
																		&& j < (routingNumber.length) - 4) {
																	stars += "*";
																}
															}
															routingNumber = stars
																	+ routingNumber
																			.substr(
																					routingNumber.length - 4,
																					routingNumber.length - 1);
															scope.clientcarddetails[i].routingNumber = routingNumber;

															var decrypted2 = CryptoJS.AES
																	.decrypt(
																			scope.clientcarddetails[i].bankAccountNumber,
																			key);
															var bankAccountNumber = decrypted2
																	.toString(CryptoJS.enc.Utf8);
															var stars = "";
															for ( var j in bankAccountNumber) {
																if (j >= 0
																		&& j < (bankAccountNumber.length) - 4) {
																	stars += "*";
																}
															}
															bankAccountNumber = stars
																	+ bankAccountNumber
																			.substr(
																					bankAccountNumber.length - 4,
																					bankAccountNumber.length - 1);
															scope.clientcarddetails[i].bankAccountNumber = bankAccountNumber;

															var decrypted3 = CryptoJS.AES
																	.decrypt(
																			scope.clientcarddetails[i].bankName,
																			key);
															scope.clientcarddetails[i].bankName = decrypted3
																	.toString(CryptoJS.enc.Utf8);
														}
													}
												});
							...
							scope.additionalDataTabFun = function() {
								...
								scope.clientAdditionalData = webStorage
										.get("client-additional-data");
								// credit card details
								scope.additionalDatas = {};
								resourceFactory.clientAdditionalResource
										.get(
												{
													clientId : routeParams.id
												},
												function(data) {
													...
													console
															.log(Object
																	.keys(dataObj).length);
													Object.keys(dataObj).length == 0 ? scope.editAdditionalDatasBtn = false
															: scope.editAdditionalDatasBtn = true;
							...
							scope.additionalAddressTabFun = function() {
								...
								resourceFactory.addressEditResource.get({
									clientId : routeParams.id,
									addressType : 'BILLING'
								}, function(data) {
							...
							scope.childsFun = function() {
								...
								/*
								 * resourceFactory.clientParentResource.get({clientId:routeParams.id},
								 * function(data) { scope.childsDatas =
								 * data.parentClientData; scope.parentCount =
								 * data.count; });
								 */
							};
							 ...
							 * if(status="ACTIVE") scope.orderMenu = true; }
							 */

							scope.getClientTemplateDocuments = function() {
								resourceFactory.templateResource.get({
									entityId : 0,
									typeId : 0
								}, function(data) {
							...
							scope.getTransactionHistoryFetchFunction = function(
									offset, limit, callback) {
								resourceFactory.transactionHistoryResource
							...
							scope.getOldTransactionHistoryFetchFunction = function(
									offset, limit, callback) {
								resourceFactory.transactionOldHistoryResource
										.getTransactionHistory({
											clientId : routeParams.id,
											offset : offset,
											limit : limit
							...
							scope.getClientDistributionFetchFunction = function(
									offset, limit, callback) {
								resourceFactory.creditDistributionResource.get(
										{
											clientId : routeParams.id,
											offset : offset,
											limit : limit
							...
							scope.getClientNetworkIpsFetchFunction = function() {
								resourceFactory.clientIpPoolingResource.get({
									clientId : routeParams.id
								}, function(data) {
							...
							scope.getTransactionHistory = function() {
								...
								scope.transactionhistory = paginatorService
										.paginate(
							...
							scope.getOldTransactionHistory = function() {
								...
								scope.transactionhistoryOld = paginatorService
										.paginate(
												scope.getOldTransactionHistoryFetchFunction,
												14);
							};

							scope.getClientDistribution = function() {
								scope.clientDistribution = paginatorService
										.paginate(
												scope.getClientDistributionFetchFunction,
												14);
							};

							scope.searchTransactionHistory123 = function(
									offset, limit, callback) {
								resourceFactory.transactionHistoryResource
										.getTransactionHistory({
											clientId : routeParams.id,
							...
							scope.searchTransactionHistory = function(
								...
								scope.transactionhistory = paginatorService
										.paginate(
												scope.searchTransactionHistory123,
												14);
							};

							scope.searchOldTransactionHistory = function(
									filterText) {
								scope.transactionhistory = paginatorService
										.paginate(
												scope.searchOldTransactionHistory,
												14);
							};
							 ...
							 * 'm_client'} , function(data) {
							 * scope.clientdatatables = data; });
							 */

							scope.getStatementsData = function(offset, limit,
									callback) {
								resourceFactory.statementResource.get({
									clientId : routeParams.id,
									offset : offset,
									limit : limit
								}, function(data) {
							...
							scope.getClientStatements = function() {
								...
								scope.states = [];
								scope.states = paginatorService.paginate(
										scope.getStatementsData, 9);
							};
							...
							scope.getClientAssociation = function() {
								resourceFactory.associationResource.get({
									clientId : routeParams.id
								}, function(data) {
							...
							scope.cancelScheduleOrder = function(id) {
								resourceFactory.OrderSchedulingResource.remove(
										{
											'clientId' : id
										}, {}, function(data) {
											// location.path('/viewclient/' +
							...
							scope.getAllTickets = function() {
								resourceFactory.ticketResource.getAll({
									clientId : routeParams.id
								}, function(data) {
									scope.tickets = data;
									scope.clientId = routeParams.id;
							...
							scope.financialsummeryTab = function() {
								...
								scope.journalsC = "";
								scope.depositC = "";
								scope.financialtransactions = paginatorService
										.paginate(
												scope.getFinancialTransactionsFetchFunction,
												14);
							};
							scope.invoicesTab = function() {
								...
								scope.journalsC = "";
								scope.depositC = "";
								scope.financialInvoices = paginatorService
										.paginate(scope.getInvoice, 14);
							};
							scope.paymentsTab = function() {
								...
								scope.journalsC = "";
								scope.depositC = "";
								scope.financialPayments = paginatorService
										.paginate(scope.getPayments, 14);
							};
							scope.adjustmentsTab = function() {
								...
								scope.journalsC = "";
								scope.depositC = "";
								scope.financialAdjustments = paginatorService
										.paginate(scope.getAdjustments, 14);
							};
							...
							scope.journalsTab = function() {
								...
								scope.depositC = "";
								scope.financialJournals = paginatorService
										.paginate(scope.getjournals, 14);
							};
							...
							scope.depositsTab = function() {
								...
								scope.financialDeposits = paginatorService
										.paginate(scope.getDeposits, 14);
							...
							scope.getEventSale = function() {
								...
								resourceFactory.eventOrderPriceUpdateTemplateResource
										.get({
											clientId : routeParams.id
										}, function(data) {
										...
										.getTableDetails(
												...
												function(data) {
													...
													scope.datatabledetails.isData = data.data.length > 0 ? true
															: false;
													scope.datatabledetails.isMultirow = data.columnHeaders[0].columnName == "id" ? true
															: false;
							...
							scope.viewDataTable = function(registeredTableName,
									data) {
								...
								if (scope.datatabledetails.isMultirow) {
									...
									location.path("/viewsingledatatableentry/"
											+ registeredTableName + "/"
											+ scope.client.id);
								}
							};
							...
							scope.deleteAll = function(apptableName, entityId) {
								resourceFactory.DataTablesResource.remove({
							 ...
							 * scope.clientdocuments = data; }); }
							 * 
							 * resourceFactory.creditCardSaveResource.get({clientId:
							 * routeParams.id} , function(data1) {
							 * 
							 * var key = mifosX.models.encrptionKey;
							 * scope.clientcarddetails = data1; for ( var i in
							 * scope.clientcarddetails) {
							 * 
							 * if(scope.clientcarddetails[i].type=='CreditCard'){
							 * 
							 * var decrypted1 =
							 * CryptoJS.AES.decrypt(scope.clientcarddetails[i].cardNumber,
							 * key); var cardNum =
							 * decrypted1.toString(CryptoJS.enc.Utf8); var stars =
							 * ""; for (var j in cardNum){ if(j>=0&&j<(cardNum.length)-4){
							 * stars += "*"; }; } cardNum =
							 * stars+cardNum.substr(cardNum.length-4,cardNum.length-1);
							 * scope.clientcarddetails[i].cardNumber = cardNum;
							 * var decrypted2 =
							 * CryptoJS.AES.decrypt(scope.clientcarddetails[i].cardExpiryDate,
							 * key); scope.clientcarddetails[i].cardExpiryDate =
							 * decrypted2.toString(CryptoJS.enc.Utf8);
							 * 
							 * }else if(scope.clientcarddetails[i].type=='ACH'){
							 * 
							 * var decrypted1 =
							 * CryptoJS.AES.decrypt(scope.clientcarddetails[i].routingNumber,
							 * key); var routingNumber =
							 * decrypted1.toString(CryptoJS.enc.Utf8); var stars =
							 * ""; for (var j in routingNumber){ if(j>=0&&j<(routingNumber.length)-4){
							 * stars += "*"; }; } routingNumber =
							 * stars+routingNumber.substr(routingNumber.length-4,routingNumber.length-1);
							 * scope.clientcarddetails[i].routingNumber =
							 * routingNumber;
							 * 
							 * var decrypted2 =
							 * CryptoJS.AES.decrypt(scope.clientcarddetails[i].bankAccountNumber,
							 * key); var bankAccountNumber =
							 * decrypted2.toString(CryptoJS.enc.Utf8); var stars =
							 * ""; for (var j in bankAccountNumber){ if(j>=0&&j<(bankAccountNumber.length)-4){
							 * stars += "*"; }; } bankAccountNumber =
							 * stars+bankAccountNumber.substr(bankAccountNumber.length-4,bankAccountNumber.length-1);
							 * scope.clientcarddetails[i].bankAccountNumber =
							 * bankAccountNumber;
							 * 
							 * var decrypted3 =
							 * CryptoJS.AES.decrypt(scope.clientcarddetails[i].bankName,
							 * key); scope.clientcarddetails[i].bankName =
							 * decrypted3.toString(CryptoJS.enc.Utf8);
							 *  } } });
							 *  };
							 */
							...
							scope.getAllOwnHardware = function() {
								scope.eventsaleC = "";
								scope.mydeviceC = "active";
								resourceFactory.HardwareResource
										.getAllOwnHardware({
											clientId : routeParams.id
										}, function(data) {
							...
							scope.cancelSale = function(otsId, index) {
								resourceFactory.deleteOneTimeSaleResource
										.remove({
											saleId : otsId
										},
												function(data) {
													scope.onetimesales.splice(
															index, 1);
													getDetails();
												}, function(errorData) {
												});
							...
							scope.unallocateDevice = function(otsId, serialNo) {
								...
								resourceFactory.unallocateDeviceResource
										.update({
											allocationId : otsId
							 ...
							 * resourceFactory.FineTransactionResource.getAllFineTransactions({clientId:
							 * routeParams.id} , function(data) {
							...
							scope.getFinancialTransactionsFetchFunction = function(
									offset, limit, callback) {
								resourceFactory.FineTransactionResource
										.getAllFineTransactions({
											clientId : routeParams.id,
											offset : offset,
											limit : limit
							...
							scope.getInvoice = function(offset, limit,
									callback, invoice) {
								resourceFactory.Filetrans.get({
									clientId : routeParams.id,
									offset : offset,
									limit : limit,
									type : scope.invoice
							...
							scope.getPayments = function(offset, limit,
									callback, payment) {
								resourceFactory.Filetrans.get({
									clientId : routeParams.id,
									offset : offset,
							...
							scope.getAdjustments = function(offset, limit,
									callback, adjustment) {
								resourceFactory.Filetrans.get({
									clientId : routeParams.id,
									offset : offset,
							...
							scope.getjournals = function(offset, limit,
									callback, adjustment) {
								resourceFactory.Filetrans.get({
									clientId : routeParams.id,
									offset : offset,
							...
							scope.getDeposits = function(offset, limit,
									callback, adjustment) {
								resourceFactory.Filetrans.get({
									clientId : routeParams.id,
									offset : offset,
									limit : limit,
									type : scope.depositAndRefund
								}, callback);
							};
							...
							scope.sendIp = function(id) {
								...
								resourceFactory.ipStatusResource.update({
									id : id
								}, {}, function(data) {
							...
							scope.searchFinancialTransactions123 = function(
									offset, limit, callback) {
								resourceFactory.FineTransactionResource
										.getAllFineTransactions({
											clientId : routeParams.id,
							...
							scope.searchFinancialTransactions = function(
									filterText) {
								scope.financialtransactions = paginatorService
										.paginate(
												scope.searchFinancialTransactions123,
												14);
							};
							...
							scope.deleteClientIdentifierDocument = function(
									clientId, entityId, index) {
								resourceFactory.clientIdenfierResource.remove({
									clientId : clientId,
									id : entityId
								}, '', function(data) {
							...
							scope.downloadClientIdentifierDocument = function(
									identifierId, documentId) {
							 ...
							 * if(query.length>0){
							 * resourceFactory.clientParentResource.get({query:
							 * query}, function(data) { scope.parentClients =
							 * data; }); }else{
							 *  } };
							 */
							...
							scope.getparent = function(query) {
								...
								return http
										.get(
												$rootScope.hostUrl
														+ '/obsplatform/api/v1/parentclient/',
												{
													params : {
														query : query
													}
							...
							scope.saveParent = function(displayLabel) {
								...
								resourceFactory.clientParentResource.update({
									clientId : routeParams.id
								}, obj, function(data) {
									location.path('/viewclient/'
											+ routeParams.id);
								...
								// webStorage.add("callingTab", {someString:
								// "moreInfo" });
							};
							scope.routeToParentClientOrChildClient = function(
									id) {
								webStorage.add("callingTab", {
									someString : "moreInfo"
								});
								webStorage.add("callingTab", {
									someString : "ChildDetailsTab"
								});
							...
							scope.fetchInventureScore = function() {
								...
								// dummy data for inventure loan recommendation
								// - DEBUG purpose
								...
								// this part is used to generate data to see the
								// look of the graph
								...
								// add the comparison chart to the
								// viewclient.html
								nv
										.addGraph(function() {
											var comparisonChart = nv.models
													.discreteBarChart().x(
															function(d) {
																return d.label;
															}).y(function(d) {
													...
													.tooltips(true).showValues(
															true);

											// set all display value to integer
											comparisonChart.yAxis.tickFormat(d3
													.format('d'));
											comparisonChart.valueFormat(d3
													.format('d'));
											comparisonChart.forceY([
													inventureMinScore,
													inventureMaxScore ]);
											...
											nv.utils
													.windowResize(comparisonChart.update);
								...
								nv.addGraph(function() {
									...
									d3.select('#inventureBulletChart svg')
											.datum(scoreData()).transition()
											.duration(1500).call(bullet);
								...
								function scoreData() {
									return {
										"title" : "",
										"ranges" : [ (inventureMinScore - 300),
												(inventureMaxScore - 300) ],
										"measures" : [ (inventureScore - 300) ],
										"markers" : [ (inventureScore - 300) ]
									};
								}

								// this will be used to display the score on the
								// viewclient.html
								scope.inventureScore = inventureScore;
							};

							var DownloadCSVFinancialDataController = function(
									$scope, $modalInstance) {

								var date = new Date(), y = date.getFullYear(), m = date
										.getMonth();
								$scope.formData = {};
								...
								$scope.accept = function() {
									var fromDate = new Date($scope.start.date)
											.getTime();
									var toDate = new Date($scope.to.date)
											.getTime();
									var downloadType = $scope.formData.downloadType;
									window
											.open($rootScope.hostUrl
													+ API_VERSION
													+ '/financialTransactions/download/'
													+ routeParams.id
													+ '?downloadType='
													+ downloadType
													+ '&fromDate=' + fromDate
							...
							function approve3($scope, $modalInstance) {
								$scope.approve = function() {
									resourceFactory.addressResource
											.remove(
													{
														clientId : scope.id
													},
							 ...
							 */
							scope.deletePopUp = function(firstId, secondId,
									index, screenName) {
							...
							function approve($scope, $modalInstance) {
								...
								$scope.approve = function() {
									...
									case 'Delete Identities':
										...
										scope.deleteClientIdentifierDocument(
												scope.idValuePopup,
												scope.secondIdValuePopup,
												scope.indexPopup);
										break;
									...
									case 'Delete Child':
										...
										scope
												.deleteChildsFromparent(scope.idValuePopup);
										break;

									case 'Cancel Sale':

										scope.cancelSale(scope.idValuePopup,
												scope.indexPopup);
										break;

									case 'Unallocate Device':

										scope.unallocateDevice(
												scope.idValuePopup,
												scope.secondIdValuePopup);
										break;
									...
									case 'Unallocate Ip':
										...
										resourceFactory.ipPoolingIpStatusResource
									...
									case 'Cancel Statement':
										scope
												.routeToCancelBill(scope.idValuePopup);
										break;
									...
									/*
									 * if(scope.screenNamePopup == 'Delete
									 * Identities'){
									 * 
									 * scope.deleteClientIdentifierDocument(scope.idValuePopup,
									 * scope.secondIdValuePopup,
									 * scope.indexPopup); }else
									 * if(scope.screenNamePopup == 'Delete
									 * Document'){
									 * 
									 * scope.deleteDocument(scope.secondIdValuePopup,
									 * scope.indexPopup); }else
									 * if(scope.screenNamePopup == 'Delete
									 * Child'){
									 * 
									 * scope.deleteChildsFromparent(scope.idValuePopup);
									 * }else if(scope.screenNamePopup == 'Cancel
									 * Sale'){
									 * 
									 * scope.cancelSale(scope.idValuePopup,
									 * scope.indexPopup); }else
									 * if(scope.screenNamePopup == 'Unallocate
									 * Device'){
									 * 
									 * scope.unallocateDevice(scope.idValuePopup,
									 * scope.secondIdValuePopup); }else
									 * if(scope.screenNamePopup == 'Unallocate
									 * Ip'){
									 * 
									 * if(this.formData == undefined ||
									 * this.formData == null){ this.formData =
									 * {"ipAddress":scope.idValuePopup,
									 * "status":'F'}; }
									 * resourceFactory.ipPoolingIpStatusResource.update({},
									 * this.formData, function(data) {
									 * route.reload(); },function(errData){ });
									 * }else if(scope.screenNamePopup == 'Cancel
									 * Statement'){
									 * 
							...
							var RefundAmountController = function($scope,
									$modalInstance, getPaymentId, getAmount) {
								$scope.formData = {};
								resourceFactory.refundAmountResource
										.get(
												{
													depositAmount : getAmount,
													depositId : scope.clientId
												},
												function(data) {
							...
							scope.orderDisconnect = function(orderId){
					          ...
					          var OrderDisconnectController = function ($scope, $modalInstance,orderId) {
					          ...
					          scope.changePlanOrder = function(planId,clientServiceId,orderId){
					        	  ...
					        	  location.path('/neworder/'+planId+'/'+scope.clientId+'/'+clientServiceId);
					        	  /*$modal.open({
					                  templateUrl: 'changePlanOrder.html',
					                  controller: ChangePlanOrderController,
					                  resolve:{
					                	  orderId : function() {
												return orderId;
											},
					          ...
					          /*function  ChangePlanOrderController($scope, $modalInstance,orderId,pdid,clientServiceId) {
					        	 
					        	  webStorage.add("orderId",orderId);
						             $scope.cancel = function () {
						                 $modalInstance.dismiss('cancel');
						           };
						           $scope.changePlanfun = function () {
						        	   location.path('/neworder/'+pdid+'/'+scope.clientId+'/'+clientServiceId);
						        	   $modalInstance.dismiss('cancel');
						           };
								};*/
					          ...
					          var OrderRenewalController = function($scope,$modalInstance,orderId){
							...
							var StaticIpPopController = function($scope,
									$modalInstance) {
								scope.errorDetails = [];
								scope.errorStatus = [];
								$scope.formData = {};
								...
								$scope.acceptStatement = function() {
									...
									resourceFactory.ipPoolingStaticIpResource
											.update(
													this.formData,
													function(data) {
														location
							...
							function depositPopController($scope,
									$modalInstance) {
								$scope.formData = {};
								resourceFactory.depositAmountResource
										.query(
												{
	...
	mifosX.ng.application.controller(
			...
			[ '$scope', 'webStorage', '$routeParams', '$route', '$location',
					'ResourceFactory', 'PaginatorService', '$http', '$modal',
					'dateFilter', 'API_VERSION', '$rootScope',
					'PermissionService', 'localStorageService', 'TENANT',
					mifosX.controllers.ViewClientController ]).run(
			function($log) {
